...
const SpecificHotel = ({ specificHotel }) => {
  ...
  const hotelTripType = localStorage.getItem(
    "hotel_trip_type",
    hotelName?.trip_type
  );
  const hotelDescription = localStorage.getItem(
    "hotelDescription",
  ...
  return (
    <div className="w-[1200px] mx-auto mt-[100px] specifichotel-wrapper">
      <div className="w-[90%] border-8 border-[#74E2E7] text-center ml-14 specifichotel-title">
      ...
      <div
        className={`container h-full  relative hover:shadow-2xl mt-[20px] specifichotel-container`}
      >
        <img
          src={specificHotel?.image ? specificHotel?.image : hotelImage}
          alt="hotel"
          className="w-1/3 mt-[7px] relative right-[12px] bottom-[4px] bg-cover rounded-2xl specifichotel-image"
        />
