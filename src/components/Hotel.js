...
const Hotel = ({ singleHotel, index, notInterested, specifichotel }) => {
  ...
  const interestedHandler = (hotelName) => {
    ...
    localStorage.setItem('hotelInclusions',JSON.stringify(hotelName?.inclusions))
  ...
  return (
    <div
      ...
      } container h-full  relative hover:shadow-2xl`}
      ref={hotelRef}
    >
      <img
      ...
      <div className="flex flex-col w-7/12 h-full">
        ...
        <div className="relative right-[15px] flex justify-center">
          <button
            onClick={() => interestedHandler(singleHotel)}
            className="bg-[#FEA116] btn3"
          >
            Interested
          </button>
          <button
            className="bg-[#FEA116] btn4"
